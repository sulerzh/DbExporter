...
namespace DbExporter.Provider.Aggram.ParadoxReader
{
    ...
    internal class ParadoxBlobFile : IDisposable
    {
        ...
        /// <summary>
        /// 根据DB文件中Blob字段的元数据信息，从MB文件中读取二进制字段
        /// </summary>
        ...
        public byte[] ReadBlob(byte[] blobInfo)
        {
            ...
            int leaderLength = blobInfo.Length - 10;

            // 解析blob's data block header in MB file  
            byte[] BlobDataHeader = new byte[10];
            Buffer.BlockCopy(blobInfo, leaderLength, BlobDataHeader, 0, 10);
            uint OffsetAndIndex = BitConverter.ToUInt32(BlobDataHeader, 0);
            uint MB_Index = OffsetAndIndex & 0x000000ff;
            uint MB_Offset = OffsetAndIndex & 0xffffff00;

            // 解析data block size
            int size = BitConverter.ToInt32(BlobDataHeader, 4);

            // modification number from the MB file header
            int mod_nr = BitConverter.ToInt16(BlobDataHeader, 8);

            if (size > 0)
            {
                if (size > leaderLength)
                {
                    // blob's data blok stored in MB file 

                    // 1、定位MB 文件指针到Blob数据所在位置
                    this.stream.Position = MB_Offset;
                    
                    // If MB_Index is FFh, then MB_Offset contains the offset of a type
                    // 02 block in the MB file.

                    // TODO 此处为Single Blob Block (Type 02)
                    ...
                    {
                        // read header
                        short blockCount = BitConverter.ToInt16(head, 1);
                        byte[] unused = new byte[9];
                        this.reader.Read(unused, 0, 9);

                        //read entry list
                        var entrys = new List<EntryInfo>();
                        for (int i = 0; i < 64; i++)
                        {
                            entrys.Add(new EntryInfo(this.reader));
                        }

                        entrys.Reverse();
                        // read data
                        MemoryStream ms = new MemoryStream();
                        foreach (EntryInfo entry in entrys)
                        {
                            if (entry.Length == 0)  break;

                            // read data block
                            this.stream.Position = MB_Offset + entry.Offset;
                ...
                {
                    // read buffer
                    byte [] leader = new byte[leaderLength];
                    Buffer.BlockCopy(blobInfo, 0, leader, 0, leaderLength);
                    byte[] buffer;
                    buffer = new byte[size];
                    Array.ConstrainedCopy(leader, 0, buffer, 0, size);
                    return buffer;
                }
            }
        ...
        /// <summary>
        /// Suballocated Block (Type 03) Entry
        /// </summary>
        internal class EntryInfo
        {
            ...
            public EntryInfo(BinaryReader r)
            {
                this.offset = r.ReadByte();
                this.lenth = r.ReadByte();
                this.modification_number = r.ReadInt16();
                this.lenth_modulo = r.ReadByte();
            }
            ...
            public int Length
            {
                get
                {
                    ...
                    {
                        return lenth * 16 - lenth_modulo % 16;
                    }
                }
            }
        }
